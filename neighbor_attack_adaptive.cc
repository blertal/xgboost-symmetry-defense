...
NeighborAttack::Result NeighborAttack::FindAdversarialPoint(
  ...
  Point &victim_label_inv   = forest_->PredictLabel(victim_point);
  for (int ii = 0; ii < config.num_features; ii++) {
    	victim_label_inv   = 1.0 - victim_label_inv;
  }

  // Find flipped images
  Point &victim_label_flip   = forest_->PredictLabel(victim_point);
  for (int ii = 0; ii < 28; ii++) {
    for (int jj = 0; jj < 28; jj++) {
        victim_label_flip[ii*28+jj]   = victim_label_flip[ii*28+28-jj];
    }
  }

  // Find flip_inverted images
  Point &victim_label_flip_inv   = victim_label_flip;
  for (int ii = 0; ii < config.num_features; ii++) {
    	victim_label_flip_inv   = 1.0 - victim_label_flip_inv;
  }
  
  int victim_label_orig     = forest_->PredictLabel(victim_point);
  int victim_label_flip     = forest_->PredictLabel(victim_point_flip);
  int victim_label_inv      = forest_->PredictLabel(victim_point_inv);
  int victim_label_flip_inv = forest_->PredictLabel(victim_point_flip_inv);
  
  
  int victim_label = -1
  if (victim_label_orig == victim_label_flip) or (victim_label_orig == victim_label_inv) or (victim_label_orig == victim_label_flip_inv):
      victim_label = victim_label_orig
  elif (victim_label_flip == victim_label_inv) or (victim_label_flip == victim_label_flip_inv):
      victim_label = victim_label_flip
  elif (victim_label_inv == victim_label_flip_inv):
      victim_label = victim_label_inv
  else:
      int rand_no = rand(0, 4)
      if (rand_no == 0){
          victim_label = victim_label_orig
      } else if (rand_no == 1){
          victim_label = victim_label_flip
          ...
          victim_label = victim_label_flip_inv
      }
