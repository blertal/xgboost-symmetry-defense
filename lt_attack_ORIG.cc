...
void BenchmarkDistortion(const Config& config) {
  ...
  int ORIG_inv_success_l2 = 0;
  int ORIG_both_success_l2 = 0;
  int ORIG_both_inv_success_l2 = 0;
  ...
  for (int row = config.offset; row < max_row; ++row) {
    ...
    //if (attack->PredictLabel(inverted) == data.first) {
      ++correct1;
    }else{
...
//    bool is_success = result.success();
    ...
    Point &adv_1   = result.best_points[1];
    Point &adv_2   = result.best_points[2];
    Point &adv_inf = result.best_points[-1];
    for (int ii = 0; ii < config.num_features; ii++) {
      adv_1[ii]   = 1.0 - adv_1[ii];
      adv_2[ii]   = 1.0 - adv_2[ii];
      adv_inf[ii] = 1.0 - adv_inf[ii];
    }


//    // From Orig Linf
    if (attack->PredictLabel(adv_2) == y_pred}//data.first){
        correct_adv1 += 1;
    }
  ...
  cout << "## Default:" << correct1 << endl;
  cout << "## Adversarial:" << correct_adv1 << endl;
  cout << "## Total samples:" << actual_num_example << endl;
