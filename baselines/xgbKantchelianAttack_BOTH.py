...
class xgbKantchelianAttack(object):
  ...
  def __init__(self, model, num_threads, json_path, order=np.inf, guard_val=GUARD_VAL, round_digits=ROUND_DIGITS, LP=False, binary=True, pos_json_input=None, neg_json_input=None):
    ...
    self.m = Model('attack')
    self.m.setParam('Threads', self.num_threads)
    if self.order != np.inf and self.order >= 2:
    	# The default 1e-4 precision will become 1e-2 on order 2, which is not acceptable.
    	print('setting OptimalityTol = 1e-8')
    	self.m.setParam('OptimalityTol', 1e-8)
    if self.LP:
...
def main(args):
  ...
  for n,idx in enumerate(samples):
    ...
    #print('non-adv (',correct1, ',', correct2,')', ', adv', adv1,',',adv2, ') - rebustness(', avg_dist, ',', counter, ')')

    if (both_model.predict(test_data[idx]) == test_labels[idx]):
        correct1 += 1
    if (both_model.predict(1-test_data[idx]) == test_labels[idx]):
        correct2 += 1
    else:
        continue

    adv = test_data[idx]#attack.attack(test_data[idx], test_labels[idx])


    if both_model.predict(adv) == test_labels[idx]:
        adv1 += 1
    if (both_model.predict(1-test_data[idx]) == test_labels[idx]) and (both_model.predict(1-adv) == test_labels[idx]):
        adv2 += 1
...
if __name__ == '__main__':
  ...
  # Not need to add zero column
  args['feature_start'] = 0
