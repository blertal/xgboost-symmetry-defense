############################################################
### Forked from https://github.com/max-andr/provably-robust-boosting
############################################################
...
def binary_search_attack(attack, f, X, y, n_trials_attack, cleanup=True):
    ...
    for i_iter_bs in range(n_iter_bs):
        f_x_vals, new_deltas = attack(f, X, y, eps, n_trials_attack, p=0.5, deltas_init=deltas, independent_delta=True)
...
class Cube(object):
    def __init__(self, model, norm_order, inverted=False, num_iterations=100):
        self.xgbmodel = model.model
        config = self.xgbmodel.config
        self.num_attack = int(config['num_attack_per_point'])
        self.num_classes = self.xgbmodel.num_classes
    ...
    def fmargin(self, X, y):
        assert np.all(y == 1)
        if self.inverted:
            margin = self.xgbmodel.inv_margin(X)
        else:
            margin = self.xgbmodel.margin(X)

        if self.num_classes > 2:
            m_correct = margin[:, self.victim_label].reshape((-1, 1))
            margin = m_correct - margin
            margin[:, self.victim_label] = np.inf
            return margin.min(axis=1)

        if self.victim_label == 1:
            return margin
        return -margin

    def __call__(self, xi, yi):
        self.victim_label = yi

        X = np.tile(xi, (self.num_attack, 1))
        y = np.ones(self.num_attack, dtype=int)

        # Cleanup to get better l1,2 norm.
        should_cleanup = (self.norm_order != np.inf)

        # 1-vs-all reduction.
        yf, deltas = binary_search_attack(cube_attack, self, X, y, self.num_iterations, cleanup=should_cleanup)
        adv_deltas = deltas[yf < 0]

        if len(adv_deltas) == 0:
            print(f'!!!Failed to attack xi:{xi} yi:{yi}!!!')
            return False, None

        norms = np.linalg.norm(adv_deltas, self.norm_order, axis=1)
        min_idx = np.argmin(norms)
        min_delta = adv_deltas[min_idx]

        return True, xi + min_delta
