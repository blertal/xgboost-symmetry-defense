...
void BenchmarkDistortion(const Config& config) {
  ...
  for (int row = config.offset; row < max_row; ++row) {
    ...
    auto& inv_data = parsed_data[row];
    
    // Find the inverted point
    Point &inverted = inv_data.second;
    for (int ii = 0; ii < config.num_features; ii++) {
      inverted[ii]   = 1.0 - inverted[ii];
    }

    cout << "Running testing example at line " << i << endl;
    int y_pred = both_attack->PredictLabel(data.second);
    //cout << "Checking if the point is correctly classified..." << endl;
    //cout << "Correct label:" << data.first << " Predict Label:" << y_pred
    //     << endl;
    if (data.first == both_attack->PredictLabel(data.second)) {
        ++correct1;
    }else{
      cout << "Mis-classified point, skipping...";
      //continue;
    }
    if (inv_data.first == both_attack->PredictLabel(inverted)) {
        ++correct2;
    }else{
      cout << "Mis-classified point, skipping...";
      continue;
    ...
    auto result = both_attack->FindAdversarialPoint(data.second);

    std::map<int, int> adv_labels;
    adv_labels[1] = both_attack->PredictLabel(result.best_points[1]);
    adv_labels[2] = both_attack->PredictLabel(result.best_points[2]);
    adv_labels[-1] = both_attack->PredictLabel(result.best_points[-1]);

    // Find the inverted sample
    //Point &inv_adv_1   = result.best_points[1];
    Point &inv_adv_2   = result.best_points[2];
    Point &inv_adv_inf = result.best_points[-1];
    for (int ii = 0; ii < config.num_features; ii++) {
      //inv_adv_1[ii]   = 1.0 - inv_adv_1[ii];
      inv_adv_2[ii]   = 1.0 - inv_adv_2[ii];
      inv_adv_inf[ii] = 1.0 - inv_adv_inf[ii];
    }

    
    if ( (both_attack->PredictLabel(data.second) == data.first) && (data.first == both_attack->PredictLabel(result.best_points[2])) ){
        ++adv1_2;
    }
    if ( (both_attack->PredictLabel(data.second) == data.first) && (data.first == both_attack->PredictLabel(result.best_points[-1])) ){
        ++adv1_inf;
    }
    
    
    if ( (both_attack->PredictLabel(inv_adv_2) == data.first) && (inv_data.first == both_attack->PredictLabel(inverted)) ){
        ++adv2_2;
    }
    if ( (both_attack->PredictLabel(inv_adv_inf) == data.first) && (inv_data.first == both_attack->PredictLabel(inverted)) ){
        ++adv2_inf;
    }


    if (config.norm_type == -1){
        if ((both_attack->PredictLabel(inverted) != data.first) || (both_attack->PredictLabel(inv_adv_inf) == data.first)){
            continue;
        }
    } else if (config.norm_type == 2){
        if ((both_attack->PredictLabel(inverted) != data.first) || (both_attack->PredictLabel(inv_adv_2) == data.first)){
            continue;
        }
    }
    ...
    if (verify_hamming) {
      ...
      int best_milp_adv_label = both_attack->PredictLabel(best_milp_adv);
      ...
      for (const auto& p : result.hist_points) {
        int hamming_dist = both_attack->HammingDistanceBetween(
            p, best_milp_adv, best_milp_adv_label, y_pred);
        best_hamming_dist = std::min(best_hamming_dist, hamming_dist);
        int neighbor_dist = both_attack->NeighborDistanceBetween(
