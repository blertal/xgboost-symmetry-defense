...
attack     = attack_list[config['search_mode']](aBothModel, norm_order)
...
for i, (xi, yi) in enumerate(test_loader):
    ...
    if (aBothModel.predict_label(xi) == yi):
        correct1 += 1
    if (aBothModel.predict_label(1-xi) == yi):
        correct2 += 1

    if not succ:
        adv = xi

    if aBothModel.predict_label(adv) == yi:
        adv1 += 1
    if aBothModel.predict_label(1-adv) == yi:
        adv2 += 1

    if (aBothModel.predict_label(1-xi) != yi) or (aBothModel.predict_label(1-adv) == yi):
        continue
    ...
    print(' non-adv (' + str(correct1) + ',' + str(correct2) + ')' + ', adv (' + str(adv1) + ',' + str(adv2) + ') - robustness( norm2=' + str(total_L2) + '/' + str(counter) + ',  norminf=' + str(total_Linf) + '/' + str(counter) + ') \n\n')
